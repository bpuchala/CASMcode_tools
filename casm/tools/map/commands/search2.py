...
The `mappings.json` output file contains:
    ...
    "options": casm.tools.map.StructureMappingSearchOptions
        The current mapping options.
    "options_history": list[casm.tools.map.StructureMappingSearchOptions]
        The history of options used for the search, in the order
        they were used.

When `casm-map search` is re-run, new results are merged with 
existing results and the options used are appended to the 
"options_history" list. Since the mappings are sorted by cost,
the methods used must not change between runs or an error is
raised.
...
def _make_options(args):
    ...
    if args.options is not None:
        ...
        options = StructureMappingSearchOptions.from_dict(data)
    else:
    ...
    print("Options:")
    print(xtal.pretty_json(options.to_dict()))
    sys.stdout.flush()
...
def run_search(args):
    ...
    # Make MappingSearchData
    if results_dir.exists():
        from casm.tools.map import MappingSearchData
        from casm.tools.shared.json_io import read_required

        data = MappingSearchData.from_dict(read_required(results_dir / "mappings.json"))
    else:
    ...
    if args.next:
        ...
        if args.alloy:
            raise ValueError("The --next option may not be used with --alloy")

        data.update_options_to_next_n_atoms(
            options=options,
        ...
        print(
            ...
            --next: 
                ...
                of atoms ({options.max_n_atoms} atoms) and merging results.
            """
        )
        sys.stdout.flush()
...
def make_search2_parser(m):
    ...
    additional.add_argument(
        ...
        help=(
            ...
            "of atoms in the parent and child structures."
        ),
    )
